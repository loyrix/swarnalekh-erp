...
- **Current phase:** B2 — Security — ✅ **DONE (2026-07-02), verified. NOT committed yet — working tree.**
- **B2 finding:** the security backend was already more complete than this tracker implied — the **audit interceptor is already global** (registered via `APP_INTERCEPTOR` in `SecurityModule`, which makes it app-wide), and `GET /security/activity-logs` (entity/action/date/search filters + PII redaction & Aadhaar/PAN masking) + `GET /security/backup` (full-tenant JSON export with counts, self-audited) already existed and are solid.
- **B2 backend fix:** `forTenant`'s docstring claimed it "filters soft-deleted records" but it didn't (doc/impl mismatch). Added pure static `PrismaService.scopeReadWhere(model, where, tenantId)` → injects `tenantId` + `deletedAt: null` for the 5 soft-delete models (Customer/InventoryItem/Invoice/SchemeEnrollment/LedgerParty) on reads (findMany/findFirst/findFirstOrThrow/count), respecting an explicit `deletedAt` so deleted rows can still be queried intentionally. `forTenant` now uses it. (Only `daily-rate` uses `forTenant` today — a model with no `deletedAt` — so this is safe-by-default hardening for future callers, not a live bug fix.) Spec `apps/api/src/prisma/prisma.service.spec.ts` (+6; api test 62 → 68).
- **B2 mobile migration:** new `features/security/data/models/activity_log.dart` (`ActivityLog` + `ActivityLogPage` w/ `latestBackup`), `data/security_repository.dart` (`SecurityQuery` value type + `getActivityLogs`/`createBackup`), `application/security_providers.dart` (`activityLogsProvider.family`). Screen **524 → ~380 lines**: `ConsumerStatefulWidget` on `AppSectionScaffold` (header = backup panel + export + search + filter icon) + `AppStateView` (4 states) + `CompactDataRow` log rows + `AppFilterSheet` (entity/action). Backup export + JSON preview fallback preserved. Removed raw-`Map` `parseActivityLogs` (typed model replaces it). New l10n `securityBackupReady` (en/hi/gu). Tests `activity_log_test.dart` + `security_screen_test.dart`. Verified: mobile analyze clean, `flutter test` = 133, api = 68.
...
- **Next action:** B3 Auth onboarding — finish + polish (password reset flow, robust register). Await owner "go" before starting.
