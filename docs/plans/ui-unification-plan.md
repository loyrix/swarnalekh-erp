...
| Billing / Invoice                                                | ⚠️ no `/preview`, stock-race, no idempotency, client-side pricing | ✅        | ✅      | B (finish)    |
...
- **Current phase:** A5 — Support screens polish (not started)
- **Last completed step:** ✅ A4 Reports complete (2026-07-01). Screen decomposed **987 → 478 lines**: 7 typed report models + `ReportsData` (`reports_data.dart`) + `ReportsRepository` (+`ReportsQuery`) + Riverpod family provider; screen now `ConsumerStatefulWidget` on `AppSectionScaffold` (Inventory/Billing/Mortgage groups) + `AppStateView`; the **6-field filter bar → `AppFilterSheet`** (search stays on-screen); report rows now render via **`CompactDataRow`** through a reusable `ReportSection` widget (+`ReportRow` view-model); typed-model→row mappers keep the screen `Map`-free; admin-only empty-state kept (guard covers `/reports`). No new l10n needed. Verified: mobile analyze clean, `flutter test` = 103 (+7), api unchanged (59).
...
- **Next action:** A5 Support screens — migrate User Management (557), Shop Profile/Tenant (524), Rates (451), Customers (555) onto the kit (typed models where missing, list rows → `CompactDataRow`, heavy forms → `AppFormScaffold` routes, `AppStateView` 4 states, l10n, tests). Rates + Tenant already have data-layer models; reuse.
